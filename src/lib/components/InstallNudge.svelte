<script lang="ts">
  ...
  function getDeferred(): BeforeInstallPromptEvent | null | undefined {
    return (window as unknown as { __deferredInstallPrompt?: BeforeInstallPromptEvent | null }).__deferredInstallPrompt
  }
  ...
  function resolve() {
    ...
    if (!installed) {
      if (!installDismissed) {
        if (canInstall) { step = 'install'; return }
        if (isIOS) { step = 'install-ios'; return }
      }
  ...
  function onInstallable() { resolve() }
  function onInstalled() { resolve() }
...
<div class="nudge-card" role="complementary" aria-label="Setup nudge">
  <!-- signal dot -->
  <div class="signal-dot" class:signal-primary={step === 'install' || step === 'install-ios'} class:signal-amber={step === 'notify' || step === 'notify-blocked'}></div>

  <!-- dismiss × button top-right -->
  <button type="button" class="btn btn-ghost btn-icon btn-sm dismiss-btn" aria-label="Dismiss" on:click={dismiss}>
  ...
  <div class="nudge-body">
    {#if step === 'install' || step === 'install-ios'}
      ...
      <div class="nudge-actions">
        {#if step === 'install'}
          <button type="button" class="btn btn-primary btn-sm" on:click={handleInstall}>Install app</button>
        {:else}
          <button type="button" class="btn btn-primary btn-sm" on:click={() => showIosInstructions = !showIosInstructions}>How?</button>
        {/if}
        <button type="button" class="btn btn-ghost btn-sm" on:click={dismissInstall}>Not now</button>
      </div>
      {#if showIosInstructions}
        <p class="ios-hint">Tap the Share button in your browser's toolbar, then choose 'Add to Home Screen.'</p>
      {/if}
    {:else if step === 'notify'}
      <p class="nudge-headline">Get pinged when an agent needs you</p>
      <p class="nudge-text">Agents keep running after you close the tab. Know the moment one finishes or needs a decision.</p>
      <div class="nudge-actions">
        <button type="button" class="btn btn-primary btn-sm" on:click={handleEnablePush} disabled={pushLoading}>Turn on notifications</button>
        <button type="button" class="btn btn-ghost btn-sm" on:click={dismissNotify}>Not now</button>
      </div>
    {:else if step === 'notify-blocked'}
      <p class="nudge-headline">Alerts are turned off</p>
      <p class="nudge-text">Re-enable notifications in your browser settings to hear from your agents.</p>
      <div class="nudge-actions">
...
@keyframes nudgePulseAmber {
  0% { box-shadow: 0 0 0 0 hsl(var(--st-needs) / 0.5); }
  70% { box-shadow: 0 0 0 8px hsl(var(--st-needs) / 0); }
  100% { box-shadow: 0 0 0 0 hsl(var(--st-needs) / 0); }
}

@media (prefers-reduced-motion: reduce) {
  .signal-primary, .signal-amber {
    animation: none;
  }
}
